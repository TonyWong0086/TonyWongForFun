document.addEventListener('DOMContentLoaded',()=>{
  ...
  const walls={
    ground:Bodies.rectangle(width/2,height+50,width*2,100,wallOptions),
    ceiling:Bodies.rectangle(width/2,-50,width*2,100,wallOptions),
  ...
  const updateStatus=()=>{const up=engine.gravity.y<0;status.textContent=up?'Upward pull':'Downward pull';reading.textContent=`${up?'−':'+'}${Math.abs(engine.gravity.y).toFixed(2)} G`;gravityBtn.innerHTML=up?'<span aria-hidden="true">⇣</span> Restore gravity':'<span aria-hidden="true">⇅</span> Reverse gravity';document.body.classList.toggle('gravity-reversed',up)};
  shakeBtn.addEventListener('click',()=>{disturb(.11);events++;eventCount.textContent=String(events).padStart(2,'0');status.textContent='Agitation event';setTimeout(updateStatus,700)});
  ...
  window.addEventListener('resize',()=>{width=chamber.clientWidth;height=chamber.clientHeight;Body.setPosition(walls.ground,{x:width/2,y:height+50});Body.setPosition(walls.ceiling,{x:width/2,y:-50});Body.setPosition(walls.left,{x:-50,y:height/2});Body.setPosition(walls.right,{x:width+50,y:height/2})});
